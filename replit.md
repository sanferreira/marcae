...
- Total-block UX: `app/index.tsx` redirects admins with an inactive plan to `app/upgrade.tsx`. Other roles can still browse (read-only) — backend enforces the write block.
